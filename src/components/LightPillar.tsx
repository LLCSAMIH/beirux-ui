...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        const int WAVE_ITERATIONS = ${4};
        const float STEP_MULT = ${(1.0).toFixed(1)};
  ...
  useEffect(() => { if (!materialRef.current) return; materialRef.current.uniforms.uTopColor.value = new THREE.Vector3(...Object.values(new THREE.Color(topColor))); }, [topColor]);
  useEffect(() => { if (!materialRef.current) return; materialRef.current.uniforms.uBottomColor.value = new THREE.Vector3(...Object.values(new THREE.Color(bottomColor))); }, [bottomColor]);

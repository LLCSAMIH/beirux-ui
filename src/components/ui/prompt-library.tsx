...
export function PromptLibraryTrigger({
  ...
  return (
    <PopoverTrigger render={<Button className={cn("gap-1.5", className)} size="sm" type="button" variant="ghost" {...props} />}>{children ?? (
                <>
                  <HugeiconsIcon
                    className="size-3.5"
                    icon={LibraryIcon}
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {children ?? (
          <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
            ...
            <DialogFooter>
              <DialogClose render={<Button type="button" variant="outline" />}>Cancel
                                          </DialogClose>
